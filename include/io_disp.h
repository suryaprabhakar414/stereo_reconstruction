...
public:
    ...
    DisparityImage (const float* data, const std::int32_t width, const std::int32_t height) : width_(width), height_(height) {
    ...
    DisparityImage (const std::int32_t width, const std::int32_t height) : width_(width), height_(height) {
        data_ = (float*)malloc(width*height*sizeof(float));
    ...
    inline float getDisp (const std::int32_t u,const std::int32_t v) {
        return data_[v*width_+u];
    }

    // is disparity valid
    inline bool isValid (const std::int32_t u,const std::int32_t v) {
        return data_[v*width_+u]>=0;
    }

    // set disparity at given pixel
    inline void setDisp (const std::int32_t u,const std::int32_t v,const float val) {
        data_[v*width_+u] = val;
    }

    // is disparity at given pixel to invalid
    inline bool setInvalid (const std::int32_t u,const std::int32_t v) {
        data_[v*width_+u] = -1;
    }
    ...
    void interpolateBackground () {
        ...
        // for each row do
        for (std::int32_t v=0; v<height_; v++) {

            // init counter
            std::int32_t count = 0;

            // for each pixel do
            for (std::int32_t u=0; u<width_; u++) {
            ...
            for (std::int32_t u=width_-1; u>=0; u--) {
                if (isValid(u,v)) {
                    for (std::int32_t u2=u+1; u2<=width_-1; u2++)
                        setDisp(u2,v,getDisp(u,v));
                    break;
                }
            }
        ...
        for (std::int32_t u=0; u<width_; u++) {
            ...
            for (std::int32_t v=height_-1; v>=0; v--) {
                if (isValid(u,v)) {
                    for (std::int32_t v2=v+1; v2<=height_-1; v2++)
                        setDisp(u,v2,getDisp(u,v));
                    break;
                }
            }
    ...
    png::image<png::rgb_pixel> errorImage (DisparityImage &D_noc,DisparityImage &D_occ,bool log_colors=false) {
        ...
        for (std::int32_t v=1; v<height()-1; v++) {
            for (std::int32_t u=1; u<width()-1; u++) {
                if (D_occ.isValid(u,v)) {
                    ...
                    for (std::int32_t v2=v-1; v2<=v+1; v2++)
                        for (std::int32_t u2=u-1; u2<=u+1; u2++)
    ...
    std::int32_t width  () { return width_; }
    std::int32_t height () { return height_; }
